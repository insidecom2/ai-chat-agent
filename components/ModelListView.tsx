  ...
  return (
    <div className="flex flex-col h-screen w-full max-w-2xl mx-auto p-4 gap-6">
      ...
      <div className="flex-1 overflow-y-auto scrollbar-thin space-y-3 pr-2">
        ...
        {!isLoading && isError && (
          <div className="bg-red-950/40 border border-red-800/50 text-red-400 p-4 rounded-xl text-sm flex items-start gap-3">
        ...
        {!isLoading && !isError && models?.length === 0 && (
          <div className="text-center py-12 text-zinc-600 text-sm">
            ...
            <code className="bg-zinc-800 px-1 rounded text-zinc-400">ollama pull &lt;name&gt;</code>
          </div>
        )}
        ...
        {!isLoading &&
          models?.map((model: any) => (
            <Card
              key={model.name}
              className="bg-zinc-900/50 border-zinc-800 hover:border-green-500/40 transition-all duration-200 cursor-pointer group active:scale-[0.98]"
              onClick={() => onSelectModel(model.name)}
            >
              <CardContent className="p-4 flex justify-between items-center">
                <div className="space-y-1">
                  <div className="flex items-center gap-2">
                    <span className="font-medium text-zinc-200 group-hover:text-green-400 transition-colors">
                      {model.name}
                    </span>
                    <Badge
                      variant="outline"
                      className="text-[10px] bg-zinc-950 text-zinc-400 border-zinc-700"
                    >
